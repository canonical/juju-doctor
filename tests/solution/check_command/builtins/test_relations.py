...
def test_builtins_failing_app():
    ...
    test_args = [
        ...
        "--verbose",
        "--format=json",
        "--probe=file://tests/resources/probes/ruleset/invalid/builtins/relations/failing-app.yaml",
...
def test_builtins_failing_endpoint():
    # GIVEN a RuleSet with failing Relations assertions
    runner = CliRunner()
    test_args = [
        "check",
        "--verbose",
        "--format=json",
        "--probe=file://tests/resources/probes/ruleset/invalid/builtins/relations/failing-endpoint.yaml",
        "--bundle=tests/resources/artifacts/bundle.yaml",
    ]
    # WHEN `juju-doctor check` is executed
    result = runner.invoke(app, test_args)
